        ...
        (function ($) {
            $(document).ready(function () {
                ...
                $('.cd-dropdown-content a').on('click', function () {
                    $('.cd-dropdown').removeClass('dropdown-is-active'); // Use the actual open class
                });
...
$(document).ready(function () {
    ...
    $(window).on('scroll', function () {
        ...
        $('[data-section-scroll]').each(function () {
            var sectionTop = $(this).offset().top - 100; // adjust for header height
            var sectionBottom = sectionTop + $(this).outerHeight();

            if (scrollPos >= sectionTop && scrollPos < sectionBottom && !found) {
                var sectionId = $(this).attr('data-section-scroll');
                $('.wd_single_index_menu ul li').removeClass('active');
                $('.wd_single_index_menu ul li a[href="' + sectionId + '"]').parent().addClass('active');
                found = true;
            }
        });

        // Edge case: scroll at top
        if (scrollPos < 100) {
            $('.wd_single_index_menu ul li').removeClass('active');
    ...
    // Trigger scroll on load to set active state
    $(window).trigger('scroll');
});



// for cursor effect js start
